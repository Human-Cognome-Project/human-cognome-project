...
TEST_P(LlvmOfflineCacheTest, ReadWrite) {
  ...
  // Both absent (legacy) and mismatched runtime markers must reject code
  // before it can be linked against the current runtime structure.
  const auto abi_path = tmp_dir / "runtime_abi";
  const auto saved_abi_path = tmp_dir / "runtime_abi.saved";
  fs::rename(abi_path, saved_abi_path);
  EXPECT_EQ(LlvmOfflineCacheFileReader::make(tmp_dir_str, llvm_fmt), nullptr);
  {
    std::ofstream incompatible(abi_path);
    incompatible << -1 << '\n';
  }
  EXPECT_EQ(LlvmOfflineCacheFileReader::make(tmp_dir_str, llvm_fmt), nullptr);
  fs::remove(abi_path);
  fs::rename(saved_abi_path, abi_path);
