...
LlvmRuntimeExecutor::LlvmRuntimeExecutor(CompileConfig &config,
    ...
    : config_(config),
      snode_capacity_(config.llvm_snode_capacity),
      snode_tree_capacity_(config.llvm_snode_tree_capacity) {
  TI_ERROR_IF(
      config.llvm_snode_capacity <= 0 || config.llvm_snode_tree_capacity <= 0,
...
void LlvmRuntimeExecutor::print_list_manager_info(void *list_manager,
  ...
  fmt::print(" length={}     {} chunks x [{} x {} B]  total={:.4f} MB\n",
             list_manager_len, num_active_chunks, elements_per_chunk,
             element_size, size_MB);
}
...
void LlvmRuntimeExecutor::print_memory_profiler_info(
  ...
  fmt::print(
      "Total requested dynamic memory (excluding alignment padding): {} B\n",
...
void LlvmRuntimeExecutor::initialize_llvm_runtime_snodes(
  ...
  TI_ERROR_IF(tree_id < 0 || tree_id >= snode_tree_capacity_,
              "SNode tree ID {} exceeds llvm_snode_tree_capacity {}; increase "
              "it in ti.init()",
              tree_id, snode_tree_capacity_);
  TI_ERROR_IF(root_id < 0 || root_id >= snode_capacity_,
              "Root SNode ID {} exceeds llvm_snode_capacity {}; increase it in "
              "ti.init()",
              root_id, snode_capacity_);
  for (const auto &meta : snode_metas) {
    TI_ERROR_IF(
        meta.id < 0 || meta.id >= snode_capacity_,
        "SNode ID {} exceeds llvm_snode_capacity {}; increase it in ti.init()",
        meta.id, snode_capacity_);
  }
  ...
  if (!all_dense) {
    // Batch only runs of IDs actually belonging to this tree. Independently
    // constructed FieldsBuilders can interleave IDs across trees.
    std::vector<int> ids;
    ids.reserve(snode_metas.size());
    for (const auto &meta : snode_metas) {
      // Field loops traverse containers; scalar places never need lists.
    ...
    for (size_t first = 0; first < ids.size();) {
      size_t end = first + 1;
      while (end < ids.size() && ids[end] == ids[end - 1] + 1) {
        ++end;
      }
      runtime_jit->call<void *, int, int>("runtime_initialize_snode_lists",
                                          llvm_runtime_, ids[first],
                                          static_cast<int>(end - first));
      first = end;
    }
  }
  runtime_jit->call<void *, std::size_t, int, int, std::size_t, Ptr, bool>(
      "runtime_initialize_snodes", llvm_runtime_, root_size, root_id, tree_id,
      rounded_size, root_buffer, all_dense);
...
void LlvmRuntimeExecutor::materialize_runtime(KernelProfilerBase *profiler,
  ...
  if (config_.arch == Arch::cuda || config_.arch == Arch::amdgpu) {
    ...
    runtime_jit->call<void *, int32_t, int32_t, int32_t, int32_t>(
        "runtime_get_memory_requirements", temp_result_ptr, num_rand_states,
        /*use_preallocated_buffer=*/1, snode_capacity_, snode_tree_capacity_);
  ...
  runtime_jit->call<void *, void *, std::size_t, void *, int, void *, void *,
                    void *, int, int>(
      ...
      (void *)std::vsnprintf, snode_capacity_, snode_tree_capacity_);
