...
#define STRUCT_FIELD_ARRAY(S, F)                                     \
  extern "C" std::remove_reference_t<decltype(((S *)nullptr)->F[0])> \
      S##_get_##F(S *s, int i) {                                     \
    return s->F[i];                                                  \
  }                                                                  \
  extern "C" void S##_set_##F(                                       \
      S *s, int i,                                                   \
      std::remove_reference_t<decltype(((S *)nullptr)->F[0])> f) {   \
    s->F[i] = f;                                                     \
  };
...
// are >= 2 ** 31 elements.
extern "C" void grid_memfence();

struct ListManager {
  static constexpr std::size_t max_num_chunks = 128 * 1024;
  // On 64-bit targets: 2 KiB of roots and a lazy 4 KiB page per 512 chunks,
  // instead of a 1 MiB embedded directory in every manager.
  static constexpr std::size_t chunks_per_page = 512;
  static constexpr std::size_t num_chunk_pages =
      max_num_chunks / chunks_per_page;
  Ptr *chunk_pages[num_chunk_pages]{};
  ...
  template <typename T>
  static T *load_directory_pointer(T **slot) {
#if ARCH_cuda || ARCH_amdgpu
    // LLVM 15 NVPTX cannot lower an acquire atomic pointer load on Pascal.
    // Use the runtime's supported integer atomic RMW, then acquire ordering.
    auto value = atomic_or_u64(reinterpret_cast<u64 *>(slot), 0);
    grid_memfence();
  ...
  template <typename T>
  static void publish_directory_pointer(T **slot, T *value) {
#if ARCH_cuda || ARCH_amdgpu
    grid_memfence();
    atomic_exchange_u64(reinterpret_cast<u64 *>(slot),
                        reinterpret_cast<u64>(value));
#else
    __atomic_store_n(slot, value, __ATOMIC_RELEASE);
#endif
  }

  Ptr get_chunk(int chunk_id) {
    auto page =
        load_directory_pointer(&chunk_pages[chunk_id / chunks_per_page]);
    return page ? load_directory_pointer(&page[chunk_id % chunks_per_page])
                : nullptr;
  }

  i32 get_num_active_chunks() {
    i32 counter = 0;
    for (int p = 0; p < num_chunk_pages; p++) {
      auto page = load_directory_pointer(&chunk_pages[p]);
      if (page) {
        for (int i = 0; i < chunks_per_page; i++) {
          counter += (load_directory_pointer(&page[i]) != nullptr);
        }
      }
    }
    return counter;
...
#if ARCH_cuda
    // The element must already exist: allocation callers have just acquired
    // its chunk through touch_chunk; traversal/GC consume an earlier task's
    // completed list. Published directory slots never move or change.
    // Volatile loads avoid stale GPU cache lines without serializing every
    // established-element lookup through an atomic RMW.
    const int chunk_id = i >> log2chunk_num_elements;
    auto page = reinterpret_cast<Ptr *volatile *>(
        chunk_pages)[chunk_id / chunks_per_page];
    auto chunk =
        reinterpret_cast<Ptr volatile *>(page)[chunk_id % chunks_per_page];
#else
    auto chunk = get_chunk(i >> log2chunk_num_elements);
#endif
    return chunk + element_size * (i & ((1 << log2chunk_num_elements) - 1));
  }
  ...
  i32 ptr2index(Ptr ptr) {
    ...
    for (int i = 0; i < max_num_chunks; i++) {
      ...
      if (chunk && chunk <= ptr && ptr < chunk + chunk_size) {
...
struct LLVMRuntime {
  ...
  Ptr *roots;
  size_t *root_mem_sizes;
  i32 snode_capacity;
  i32 snode_tree_capacity;

  Ptr thread_pool;
  parallel_for_type parallel_for;
  ListManager **element_lists;
  NodeManager **node_allocators;
  Ptr *ambient_elements;
  Ptr temporaries;
...
void runtime_get_memory_requirements(Ptr result_buffer,
  ...
  size += taichi::iroundup(i64(sizeof(Ptr)) * snode_tree_capacity,
                           taichi_page_size);
  size += taichi::iroundup(i64(sizeof(size_t)) * snode_tree_capacity,
                           taichi_page_size);
  size +=
      3 * taichi::iroundup(i64(sizeof(Ptr)) * snode_capacity, taichi_page_size);
...
void runtime_initialize(
  ...
  runtime->snode_capacity = snode_capacity;
  runtime->snode_tree_capacity = snode_tree_capacity;
  runtime->roots = (Ptr *)runtime->allocate_aligned(
      runtime->runtime_objects_chunk, sizeof(Ptr) * snode_tree_capacity,
      taichi_page_size);
  runtime->root_mem_sizes = (size_t *)runtime->allocate_aligned(
      runtime->runtime_objects_chunk, sizeof(size_t) * snode_tree_capacity,
      taichi_page_size);
  runtime->element_lists = (ListManager **)runtime->allocate_aligned(
      runtime->runtime_objects_chunk, sizeof(Ptr) * snode_capacity,
      taichi_page_size);
  runtime->node_allocators = (NodeManager **)runtime->allocate_aligned(
      runtime->runtime_objects_chunk, sizeof(Ptr) * snode_capacity,
      taichi_page_size);
  runtime->ambient_elements = (Ptr *)runtime->allocate_aligned(
      runtime->runtime_objects_chunk, sizeof(Ptr) * snode_capacity,
      taichi_page_size);
  for (int i = 0; i < snode_tree_capacity; ++i) {
    runtime->roots[i] = nullptr;
    runtime->root_mem_sizes[i] = 0;
  }
  for (int i = 0; i < snode_capacity; ++i) {
    runtime->element_lists[i] = nullptr;
    runtime->node_allocators[i] = nullptr;
    runtime->ambient_elements[i] = nullptr;
  }
...
void runtime_initialize_snode_lists(LLVMRuntime *runtime,
                                    int first_id,
                                    int count) {
  for (int i = 0; i < count; ++i) {
    runtime->element_lists[first_id + i] =
...
void ListManager::touch_chunk(int chunk_id) {
  if (chunk_id < 0 || chunk_id >= max_num_chunks) {
    taichi_assert_runtime(runtime, false, "List manager out of chunks.");
    return;
  }
  if (!get_chunk(chunk_id)) {
    locked_task(&lock, [&] {
      auto page_id = chunk_id / chunks_per_page;
      auto page = load_directory_pointer(&chunk_pages[page_id]);
      if (!page) {
        page = reinterpret_cast<Ptr *>(runtime->allocate_aligned(
            runtime->runtime_memory_chunk, chunks_per_page * sizeof(Ptr), 4096,
            true /*request*/));
        std::memset(page, 0, chunks_per_page * sizeof(Ptr));
        // Publish initialization before readers can observe the page.
        publish_directory_pointer(&chunk_pages[page_id], page);
      }
      auto slot = &page[chunk_id % chunks_per_page];
      // May have been allocated during lock contention.
      if (!load_directory_pointer(slot)) {
        ...
        publish_directory_pointer(slot, chunk_ptr);
      }
    });
  }
}
