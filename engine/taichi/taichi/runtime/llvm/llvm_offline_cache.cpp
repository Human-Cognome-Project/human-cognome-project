...
constexpr char kRuntimeAbiFilename[] = "runtime_abi";
...
bool LlvmOfflineCacheFileReader::load_meta_data(
  ...
  // Check before deserializing or loading any generated code. AOT artifacts
  // from the old runtime have no marker and must be regenerated.
  auto load_compatible_metadata = [&]() {
    std::ifstream abi_file(
        taichi::join_path(cache_file_path, kRuntimeAbiFilename));
    int abi = 0;
    if (!(abi_file >> abi) || abi != kLlvmRuntimeAbiVersion) {
      TI_WARN("LLVM runtime ABI mismatch in {}; regenerate the AOT module",
              cache_file_path);
      return false;
    }
  ...
  if (!with_lock) {
    return load_compatible_metadata();
  }
...
void LlvmOfflineCacheFileWriter::dump(const std::string &path,
...

  {
    ...
    std::ofstream abi_file(taichi::join_path(path, kRuntimeAbiFilename));
    abi_file << kLlvmRuntimeAbiVersion << '\n';
    abi_file.close();
    TI_ERROR_IF(!abi_file, "Failed to write LLVM runtime ABI marker in {}",
                path);
  }
