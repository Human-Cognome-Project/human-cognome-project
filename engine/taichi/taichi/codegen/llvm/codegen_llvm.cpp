...
void TaskCodeGenLLVM::visit(ExternalPtrStmt *stmt) {
  ...
  */
  ...
  // linear_index is accumulated in i64 (rather than i32) so that flattened
  // ndarray indices beyond ~2^31 elements do not overflow. Per-axis sizes
  // and index operands are still produced as i32 elsewhere (unchanged); we
  // sign-extend them here right before folding them into the i64 product.
  auto linear_index = tlctx->get_constant(get_data_type<int64>(), (int64)0);
  ...
  for (int i = 0; i < num_indices; i++) {
    if (i >= element_shape_index_offset &&
      ...
      llvm::Value *size_var = tlctx->get_constant(
          get_data_type<int64>(),
          (int64)stmt->element_shape[i - element_shape_index_offset]);
      linear_index = builder->CreateMul(linear_index, size_var);
    } else {
      // Indexing array dimensions
      llvm::Value *size_var = builder->CreateSExt(
          sizes[size_var_index++], llvm::Type::getInt64Ty(*llvm_context));
      linear_index = builder->CreateMul(linear_index, size_var);
    }
    llvm::Value *index_val = builder->CreateSExt(
        llvm_val[stmt->indices[i]], llvm::Type::getInt64Ty(*llvm_context));
    linear_index = builder->CreateAdd(linear_index, index_val);
  }
