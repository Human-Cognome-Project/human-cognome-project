...
void StructCompilerLLVM::run(SNode &root) {
  ...
  for (const auto *snode : snodes) {
    TI_ERROR_IF(
        snode->id < 0 || snode->id >= config_.llvm_snode_capacity,
        "SNode ID {} exceeds llvm_snode_capacity {}; increase it in ti.init()",
        snode->id, config_.llvm_snode_capacity);
  }
  TI_ERROR_IF(root.get_snode_tree_id() < 0 ||
                  root.get_snode_tree_id() >= config_.llvm_snode_tree_capacity,
