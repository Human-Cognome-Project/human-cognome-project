...
void CompileConfig::apply_llvm_runtime_environment() {
  auto capacity = [](const char *name, int fallback) {
    ...
    auto result = std::from_chars(value, end, parsed);
    TI_ERROR_IF(result.ec != std::errc() || result.ptr != end || parsed <= 0,
                "{} must be a positive integer representable by int", name);
    return parsed;
  };
  // Validate both options before changing either one.
  const int nodes = capacity("TI_LLVM_SNODE_CAPACITY", llvm_snode_capacity);
  const int trees =
      capacity("TI_LLVM_SNODE_TREE_CAPACITY", llvm_snode_tree_capacity);
  llvm_snode_capacity = nodes;
  llvm_snode_tree_capacity = trees;
}
