"""ARCHIVED: Working copy of Unicode text ingestion script.
...
Creates tokens under AA.AB.AA.{category}.{count} for Unicode characters,
...
from src.hcp.core.token_id import encode_token_id
from src.hcp.db.postgres import connect, init_schema, insert_token, insert_scope, insert_namespace

# Universal text encoding scope for Unicode characters: AA.AB.AA
SCOPE_UNICODE_TEXT = encode_token_id(0, 1, 0)
...
def unicode_text_token_id(category_id: int, index: int) -> str:
    ...
    return encode_token_id(0, 1, 0, category_id, index)
...
def ingest_unicode_text(conn, source_path: Path = None):
    ...
    if source_path is None:
        source_path = Path(__file__).parent.parent.parent / "sources" / "unicode_table.txt"
    ...
    with conn.cursor() as cur:
        for cat_name, cat_id in category_map.items():
            scope_id = encode_token_id(0, 1, 0, cat_id)
